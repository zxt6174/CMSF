...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            images, image_lengths, captions, lengths, ids = data_i #这里没有增加ids，源代码自带的就有ids变量，只是没有用
        else:
            images, captions, lengths, ids = data_i
        model.logger = val_logger

        # compute the embeddings # BLD  # BTLD

        if not backbone:
            img_emb, cap_emb,T_img_emb,T_cap_emb,F_img_emb,F_cap_emb= model.forward_emb(images, captions, lengths, image_lengths=image_lengths)
        else:
            img_emb, cap_emb,T_img_emb,T_cap_emb,F_img_emb,F_cap_emb = model.forward_emb(images, captions, lengths)
        ...
        if img_embs is None:
            ...
            ids_all = [0]*len(data_loader.dataset)
        # cache embeddings
        '''
        ...
        '''
        #换成cross fine的
        ##换成img_cro_emeb和cap_cro_emb会怎么样呢
        img_embs[ids] = img_emb.data.cpu().numpy().copy()
    ...
    return img_embs, cap_embs,  img_lens, cap_lens #都不用想，这里也得汇总一下T-img-embs，但是好像TBLD维度占得内存会很大呢
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    if cxc:
        ...
        if not fold5:
            ...
            #1从sims里挑选top-k个样本
            #2将对应的id对应的特征，输入到cross-attention,注意是带有时间维度的特征
            ...
            for i,sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)): 
                ...
                output = model.cross_encoder(T_img_emb,T_cap_emb,idx)
            ...
            r, rt = i2t(npts, score_matrix_i2t, return_ranks=True)
            ri, rti = t2i(npts, score_matrix_t2i, return_ranks=True)
...
def compute_sim(img_embs, cap_embs, img_lens, cap_lens, model, shard_size=128):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            sim = model.forward_sim(im, s, il, sl)#调用model.forward-sims
...
def t2i(npts, sims, return_ranks=False, mode='coco'):
    ...
    # --> (5N(caption), N(image))
    #print("t2i-npts",npts,flush=True)
    for index in range(npts):
        if mode == 'coco':
            for i in range(5):
