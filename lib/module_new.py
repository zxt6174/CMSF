...
def get_fgsims(imgs, caps):
    ...
    sims = sims.reshape(bi, n_r, bc, n_w).permute(0,2,1,3)#bi，bc，nr， nw
    return sims
...
def SCAN_attention(query, context, smooth=1):
    """
    ...
    #print("context has nan:", torch.isnan(context).any().item())

    #print("attn-begin",attn,flush=True)
    #多种选项
    attn = nn.LeakyReLU(0.1)(attn)#可以去掉，因为我们输入的值一定是整数，RELU不起作用
    attn = l2norm(attn, 2)
    #attn = l1norm(attn, 2)
    #print("attn-relu",attn,flush=True)
    # --> (batch, queryL, sourceL)
    attn = torch.transpose(attn, 1, 2).contiguous()
    # --> (batch, queryL, sourceL
    attn = F.softmax(attn*smooth, dim=2)#是不是你导致的loss=nan
    #attn = torch.sigmoid(attn * smooth)
    ...
    #print("attn-softmax:",attn,flush=True)
    #print("attn:",attn,flush=True)

    return weightedContext, attn


def Spike_SCAN_attention(query, context, smooth=9):
    """
    query: (n_context, queryL, d)
    context: (n_context, sourceL, d)
    """
