...
def encode_data_new(model, data_loader, butd, use_gpu=False):
...
def i2t_new(images, sentences, similarity_fn, nreps=1, npts=None, return_ranks=False, use_gpu=False, is_cosine=True):
    """
    ...
    for index in range(npts):
        ...
        if use_gpu:
            if len(sentences.shape) == 2:
                ...
                _, K, D = im.shape
                sim = similarity_fn(im.view(-1, D), sentences.view(-1, D)).flatten()#sim-fn输入的维度是BL，D，BN,D
        else: 
...
def t2i_new(images, sentences, similarity_fn, nreps=1, npts=None, return_ranks=False, use_gpu=False, is_cosine=True):
    """
...
def evalrank_new(model, args, split='test'):
...
if __name__ == '__main__':
    ...
    with open('%s_vocab.pkl' % args.data_name, 'rb') as f:
        vocab = pickle.load(f)
