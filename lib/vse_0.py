...
class CMSF(nn.Module):
    ...
    def train_emb(self, images, captions, lengths, image_lengths=None, 
        ...
        if self.opt.use_moco:
            ...
            loss_itct=self.forward_loss(cap_emb, img_emb)
            #loss_info =self.hal_loss.info_nce_forward(img_emb,cap_emb)

            loss = loss_itc + loss_moco+loss_itct
            ...
            if self.Eiters %100==0:
                if not dist.is_initialized() or dist.get_rank() == 0:#只让主进程print信息就行了
                    print("Loss-total:",loss.data.item(),"Loss-ITC:",loss_itc.data.item(),"Loss-moco:",loss_moco.data.item(),"Loss-itct:",loss_itct.data.item(),flush=True)
            
        else:
            img_emb, cap_emb = self.forward_emb(images, captions, lengths, image_lengths=image_lengths, is_train=True)
            loss_itc = self.forward_loss(img_emb, cap_emb)
            loss_info=self.hal_loss.info_nce_forward(img_emb,cap_emb)
            loss=loss_itc+loss_info
            if self.Eiters % 100==0:

                if not dist.is_initialized() or dist.get_rank() == 0:
                    print("Loss-total:", loss.data.item(), "Loss-itc:", loss_itc.data.item(),"Loss-info:", loss_info.data.item(),flush=True)
