...
from lib.spike_coder import Dynamic_Threshold_LIFNode,ConvEncoder,RepeatEncoder,DeltaEncoder,StageEncoder,StageTextEncoder,LinearEncoder,RepeatTextEncoder,LinearLNEncoder
...
class Spikformer(nn.Module):
    ...
    def __init__(self,
        ...
        #self.txt_attn_pool=AttentionPool(embed_dims)
    ...
    def forward(self, x,lengths):
        ...
        x=self.spike_coder(x)#娴偣杞剦鍐?        #x=self.gac(x,x_s)

        #x=self.pos_emb(x)#鍔犲叆浣嶇疆缂栫爜

        x = self.forward_features(x)#娉ㄦ剰鍔涘潡
        ...
        #x = x.flatten(1,2)
        x=l2norm(x,dim=-1)#浣跨敤l2鍙互淇濊瘉闀垮緱宸笉澶氾紝浣嗘槸鏁版嵁鏄惁鏈夋剰涔夊憿锛屾墍浠ヤ笉濡傛帴涓€涓狦PO
