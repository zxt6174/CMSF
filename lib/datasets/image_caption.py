...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenzier, opt, train):
        ...
        if 'coco' in data_name:
            ...
            self.image_base = osp.join(data_path, 'flickr30k_images')
        ...
        print("### Get the image ids###")
        with open(osp.join(loc_image, '{}_ids.txt'.format(data_split)), 'r',encoding='utf-8') as f:
...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train):
        ...
        loc_cap = osp.join(data_path, self.data_name+'_precomp')
        loc_image = osp.join(data_path, self.data_name+'_precomp')
...
def process_caption(tokenizer, tokens, train=True):
    ...
    #固定输出维度，太长就截断 ，太短就填充0,也就是PAD，即长度==36
    max_len=36
    if len(output_tokens) > max_len:
        output_tokens = output_tokens[:max_len - 1] + ['[SEP]']  # Ensure [SEP] remains at the end
    # Pad if too short
    padding_length = max_len - len(output_tokens)
    if padding_length > 0:
        output_tokens += ['[PAD]'] * padding_length
    
    #使用截断的方法， 太长就截断，但不pad，即长度<=36
...
def get_loader(data_path, data_name, data_split, tokenizer, opt, batch_size=100,
               shuffle=True, num_workers=0, train=True, clip_model = None):
...
def get_val_loader(data_path, data_name, data_split, tokenizer, opt, batch_size=100,
               shuffle=True, num_workers=0, train=True, clip_model = None):
    """Returns torch.utils.data.DataLoader for custom coco dataset."""
    if train:
        drop_last = True
...
def get_loaders(data_path, data_name, tokenizer, batch_size, workers, opt):
    ...
    val_loader = get_val_loader(data_path, data_name, 'test', tokenizer, opt,
...
def get_test_loader(split_name, data_name, tokenizer, batch_size, workers, opt):
    test_loader = get_val_loader(opt.data_path, data_name, split_name, tokenizer, opt,
                             batch_size, False, workers, train=False)
    return test_loader
